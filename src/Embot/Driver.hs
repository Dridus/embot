...
import ClassyPrelude (error, fail, forM_)
...
import Embot.Types
    ( EmbotConfig, apiToken, configurationError, setupLogging, withEnvironment
...
embotMain config = do
    ...
    inLoggingIO . $logInfo $ "Embot " <> show version <> " starting"
    ...
    view withEnvironment config $ \ logic -> do
        ...
        readerThreadId <- liftIO $ forkFinally
            ...
            (\ result -> inLoggingIO $ case result of
                Left exception -> do
                    $logError $ "reader thread failed: " <> show exception
                Right () -> do
